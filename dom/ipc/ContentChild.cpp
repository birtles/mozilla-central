...
NS_INTERFACE_MAP_BEGIN(ContentChild)
  NS_INTERFACE_MAP_ENTRY(mozilla::dom::nsIContentChild)
...
{
    return nsIContentChild::AllocPBrowserChild(aContext, aChromeFlags);
}

bool
ContentChild::SendPBrowserConstructor(PBrowserChild* actor,
                                      const IPCTabContext& context,
                                      const uint32_t& chromeFlags)
{
    return PContentChild::SendPBrowserConstructor(actor, context, chromeFlags);
}
...
{
    return nsIContentChild::DeallocPBrowserChild(iframe);
}
...
PBlobChild*
ContentChild::SendPBlobConstructor(PBlobChild* actor,
                                   const BlobConstructorParams& params)
{
    return PContentChild::SendPBlobConstructor(actor, params);
}
